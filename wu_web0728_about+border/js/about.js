...
let tops = panels.map(panel => ScrollTrigger.create({ trigger: panel, start: "top 78px" }));
...
gsap.to(".a_index", {
  y: 0,
  scrollTrigger: {
    trigger: ".panel_with_a_index", // 將觸發器改為 .panel_with_a_index
    start: "top top", // 開始動畫時 .panel_with_a_index 的頂部與視窗頂部對齊
    endTrigger: "#container", // 當 #container 進入視窗時結束動畫
    end: "bottom bottom", // 結束動畫時 .panel_with_a_index 的底部與視窗底部對齊
    scrub: true,
    onUpdate: (self) => {
      if (self.isScrubbing()) {
